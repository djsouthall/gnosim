...
class Sim:
    ...
    def detector(self):
        ...
        for key in self.config['antenna_definitions'].keys():
            self.lib[key] = gnosim.trace.refraction_library_beta.RefractionLibrary(self.config['antenna_definitions'][key]['lib'])
                ...
                if numpy.any(flag_array):
                    ...
                    for ii, solution in enumerate(self.stations[index_station].antennas[index_antenna].lib.solutions):
                        if flag_array[ii]:
                            ...
                            electric_field = self.stations[index_station].antennas[index_antenna].electricField(frequency, electric_field, dic_array[ii]['theta_ant']) # V m^-1 #THIS WAS CHANGED THETA WAS ADDED
                            #electric_field = gnosim.sim.detector.Antenna.electricField(self.config, frequency, electric_field, theta_ant)
...
if __name__ == "__main__":
    ...
    outfile = 'results_2017_may_%s_%.2e_GeV_%i_events_%i.h5'%(config_file_fix.replace('.py', ''),
                                                                energy_neutrino,
                                                                n_events,
                                                                index)
